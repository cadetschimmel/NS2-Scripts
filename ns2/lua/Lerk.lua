...
function Lerk:RedirectVelocity(redirectDir)
	...
	// Factor in half of redirected velocity and current y-velocity.
	//redirectDir.y = redirectDir.y * 0.5
	//if (redirectDir.y > 0 and velocity.y > 0) or (redirectDir.y <= 0 and velocity.y <= 0) then
	//	ySpeed = velocity.y
	//else
	//	ySpeed = redirectDir.y * velocity:GetLength()// + velocity.y
	//end
...
function Lerk:PreUpdateMove(input, runningPrediction)
    ...
    if not self:GetIsOnGround() then
        ...
        if self.gliding then                        			
            // glide foward, strafe left/right, or brake slowly
            if (move.z ~= 0) then
                ...
                //redirectDir.y = Clamp(math.max(redirectDir.y, velocity.y), 0, 1)                
            end
...
function Lerk:GetZoneDamage(damage, endPoint)
	...
	local distance = (center - endPoint):GetLength()
	
	// only 75% damage at wings
	if (distance > Lerk.kBodyRange) then
